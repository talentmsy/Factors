...
comp<-data.comp.funda[,.(gvkey,datadate,year,noa,count)]
###################
# CRSP Block      #
###################
...
comp1<-comp[,.(gvkey,datadate,noa,count)]
comp1<-comp1[is.na(noa)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,noa,count)]
...
nyse_bm1<-nyse[,quantile(noa,.1),by=jdate]
nyse_bm2<-nyse[,quantile(noa,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(noa<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(noa>bm30&noa<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(noa>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,NOAH=WH,NOAL=WL,NOA=WHML)]
write.csv(factor,file="Noa_revised.csv")
