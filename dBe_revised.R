...
data.comp.funda[,dbe:=ceq-lag(ceq,1),by=gvkey]
data.comp.funda[,dbe:=dbe/lag(at,1),by=gvkey]
data.comp.funda<-data.comp.funda[!is.na(dbe)]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,dbe,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,dbe,count)]
...
nyse_bm1<-nyse[,quantile(dbe,.1),by=jdate]
nyse_bm2<-nyse[,quantile(dbe,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(dbe<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dbe>bm30&dbe<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dbe>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dBEH=WH,dBEL=WL,dBE=WHML)]
write.csv(factor,file="dBe_revised.csv")
