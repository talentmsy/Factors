...
data.comp.funda<-data.comp.funda[xrd>0]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,xrd,count)]
###################
# CRSP Block      #
###################
...
comp1<-comp[,.(gvkey,datadate,xrd,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,xrd,count)]

# link comp and crsp
ccm_jun<-ccm2[crsp_jun,on=.(permno,jdate),nomatch=0]
ccm_jun<-ccm_jun[,rdm:=xrd/dec_me]
...
nyse_bm1<-nyse[,quantile(rdm,.1),by=jdate]
nyse_bm2<-nyse[,quantile(rdm,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(rdm<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(rdm>bm30&rdm<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(rdm>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,RDMH=WH,RDML=WL,RDM=WHML)][120:702]
write.csv(factor,file="Rdm_revised.csv")
