...
data.comp.funda[,gpa:=(revt-cogs)/at,by=gvkey]
data.comp.funda<-data.comp.funda[is.na(gpa)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,gpa,count)]
###################
# CRSP Block      #
###################
...
comp1<-comp[,.(gvkey,datadate,gpa,count)]
comp1<-comp1[is.na(gpa)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,gpa,count)]
...
nyse_bm1<-nyse[,quantile(gpa,.1),by=jdate]
nyse_bm2<-nyse[,quantile(gpa,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(gpa<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(gpa>bm30&gpa<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(gpa>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,GPAH=WH,GPAL=WL,GPA=WHML)]
write.csv(factor,file="Gpa_revised.csv")
