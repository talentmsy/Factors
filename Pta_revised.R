...
data.comp.funda<-data.comp.funda[is.na(ni)==FALSE&!ni==0]
data.comp.funda<-data.comp.funda[!(is.na(dlc-lag(dlc))&is.na(ivst-lag(ivst))&is.na(ivao-lag(ivao))&is.na(dltt-lag(dltt))&is.na(pstk-lag(pstk)))]

data.comp.funda[,wc:=act-che-lct+dlc]
data.comp.funda[,dwc:=wc-lag(wc,1),by=gvkey]

data.comp.funda[,nca:=at-act-ivao]
data.comp.funda[,ncl:=lt-lct-dltt]
data.comp.funda[,nco:=nca-ncl]
data.comp.funda[,dnco:=nco-lag(nco),gvkey]

data.comp.funda[,Fna:=ivst+ivao]
data.comp.funda[,Fnl:=dltt+dlc+pstk]
data.comp.funda[,Fin:=Fna-Fnl]
data.comp.funda[,dfin:=Fin-lag(Fin),gvkey]

data.comp.funda[year<1988,ta:=dwc+dnco+dfin,by=gvkey]
data.comp.funda[year>=1988,ta:=ni-oancf-ivncf-fincf+ifelse(is.na(sstk)==TRUE,0,sstk)-ifelse(is.na(prstkc)==TRUE,0,prstkc)-ifelse(is.na(dv)==TRUE,0,dv)]
data.comp.funda[,pta:=ta/abs(ni),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(pta)==FALSE]
# number of years in Compustat
data.comp.funda<-data.comp.funda[order(gvkey,datadate)]
data.comp.funda[,count:=ave(gvkey==gvkey, gvkey, FUN=cumsum)]
data.comp.funda[,count:=count-1]
comp<-data.comp.funda[,.(gvkey,datadate,year,pta,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,pta,count)]
...
nyse<-ccm_jun[exchcd==1&me>0&count>1&(shrcd==10|shrcd==11)]
# size breakdown
nyse_sz<-nyse[,median(me),by=jdate]
setnames(nyse_sz,"V1","sizemedn")
# beme breakdown

nyse_bm1<-nyse[,quantile(pta,.1),by=jdate]
nyse_bm2<-nyse[,quantile(pta,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,szport:=ifelse(me<=sizemedn,"S","B")]
ccm1_jun[is.na(szport)==TRUE,szport:=""]
# assign book-to-market portfolio
ccm1_jun[me>0&count>=1,bmport:=ifelse(pta<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(pta>bm30&pta<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(pta>bm70,"H",bmport)]
ccm1_jun[is.na(bmport)==TRUE,bmport:=""]

# create positivebmeme and nonmissport variable
ccm1_jun[,posbm:=ifelse(me>0&count>=1,1,0)]
...
june1<-june[,.(permno,ffyear,szport,bmport,posbm,nonmissport)]
ccm3<-june1[crsp3,on=.(permno,ffyear)]
# keeping only records that meet the criteria
ccm4<-ccm3[wt>0&posbm==1&nonmissport==1&(shrcd==10|shrcd==11)]
############################
# Form Factors #
###########################
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,PTAH=WH,PTAL=WL,PTA=WHML)]
write.csv(fffactor,file="Pta_revised.csv")
