...
data.comp.funda<-data.comp.funda[xad>0,.(gvkey,datadate,year,xad,count)]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,xad,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,xad,count)]

# link comp and crsp
ccm_jun<-ccm2[crsp_jun,on=.(permno,jdate),nomatch=0]
ccm_jun[,nxad:=xad/dec_me]
...
nyse_bm1<-nyse[,quantile(nxad,.1),by=jdate]
nyse_bm2<-nyse[,quantile(nxad,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(nxad<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(nxad>bm30&nxad<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(nxad>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,ADMH=WH,ADML=WL,ADM=WHML)][97:666]
write.csv(factor,file="Adm_revised.csv")
