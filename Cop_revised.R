...
data.comp.funda[,dr:=drc+drlt]
data.comp.funda[,cop:=revt-cogs-xsga+ifelse(is.na(xrd)==TRUE,0,xrd)
                -ifelse(is.na(rect-lag(rect,1))==TRUE,0,rect-lag(rect,1))
                -ifelse(is.na(invt-lag(invt,1))==TRUE,0,invt-lag(invt,1))
                -ifelse(is.na(xpp-lag(xpp,1))==TRUE,0,xpp-lag(xpp,1))
                +ifelse(is.na(dr-lag(dr))==TRUE,0,dr-lag(dr))
                +ifelse(is.na(ap-lag(ap,1))==TRUE,0,ap-lag(ap,1))
                +ifelse(is.na(xacc-lag(xacc,1))==TRUE,0,xacc-lag(xacc,1)),gvkey]
data.comp.funda[,cop:=cop/at,gvkey]
data.comp.funda<-data.comp.funda[is.na(cop)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,cop,count)]
...
comp1<-comp[,.(gvkey,datadate,cop,count)]
comp1<-comp1[is.na(cop)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,cop,count)]
...
nyse_bm1<-nyse[,quantile(cop,.1),by=jdate]
nyse_bm2<-nyse[,quantile(cop,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(cop<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(cop>bm30&cop<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(cop>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,COPH=WH,COPL=WL,COP=WHML)]
write.csv(factor,file="Cop_revised.csv")
