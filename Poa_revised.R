...
data.comp.funda<-data.comp.funda[!ni==0]
data.comp.funda[,dca:=act-lag(act,1),by=gvkey]
data.comp.funda[,dcash:=che-lag(che,1),by=gvkey]
data.comp.funda[,dcl:=lct-lag(lct,1),by=gvkey]
data.comp.funda[,dstd:=dlc-lag(dlc,1),by=gvkey]
data.comp.funda[,dtp:=txp-lag(txp,1),by=gvkey]
data.comp.funda[,dtp:=ifelse(is.na(dtp)==TRUE,0,dtp)]
data.comp.funda[year<1988,oa:=dca-dcash-(dcl-dstd-dtp)-dp,by=gvkey]
data.comp.funda[year>=1988,oa:=ni-oancf]
data.comp.funda[,poa:=oa/abs(ni),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(poa)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,poa,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,poa,count)]
...
nyse_bm1<-nyse[,quantile(poa,.1),by=jdate]
nyse_bm2<-nyse[,quantile(poa,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(poa<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(poa>bm30&poa<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(poa>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,POAH=WH,POAL=WL,POA=WHML)]
write.csv(factor,file="Poa_revised.csv")
