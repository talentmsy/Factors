...
comp<-data.comp.fundq[,.(gvkey,datadate,fqtr,rdq,year,roa)]
...
mycomp[,nagroup:=cumsum(!is.na(roa)),gvkey]
mycomp[,nanum:=length(jdate),by=.(gvkey,nagroup)]

mycomp[,pos:=1:.N,.(gvkey,nagroup)]
mycomp[pos<=5,roa_forward_fill:=roa[1],.(gvkey,nagroup)]
...
mycomp[,lastroa:=roa_forward_fill[replace(seq(.N) - pos, seq(.N) <= pos, NA)], gvkey]
mycomp[,lastroa:=ifelse(is.na(roa_forward_fill),NA,lastroa)]
mycomp[,newroa:=ifelse(jdate>rdq_forward_fill,roa_forward_fill,NA)]
mycomp[,newroa:=ifelse(jdate<=rdq_forward_fill,lastroa,newroa)]
mycomp[year(jdate)<1972,newroa:=lag(roa_forward_fill,3),gvkey]
mycomp[,lagroa:=lag(newroa,1),gvkey]
mycomp<-mycomp[diffinMon<=5&is.na(lastroa)==FALSE]
mycomp<-mycomp[pos<=5,.SD,.(gvkey,nagroup)]
setkeyv(mycomp,c("gvkey","jdate"))
mycomp<-unique(mycomp[is.na(lagroa)==FALSE,.(gvkey,nagroup,jdate,lagroa)])
###################
# CRSP Block      #
###################
...
ccmerged<-ccmerged[is.na(lagroa)==FALSE&is.na(lagme)==FALSE]
ccmerged<-ccmerged[,.(jdate,exchcd,retadj,lagroa,me,lagme,permno)]
rfp<-ccmerged
...
nyse_d1<-nyse[,quantile(lagroa,.1),by=jdate]
nyse_d10<-nyse[,quantile(lagroa,.9),by=jdate]
...
crsp_factor[,dport:=ifelse(lagroa<=d10,"L","")]
crsp_factor[,dport:=ifelse(lagroa>d10&lagroa<=d90,"M",dport)]
crsp_factor[,dport:=ifelse(lagroa>d90,"H",dport)]
...
factor[,WL:=(BL+SL)/2]
factor[,WHML:=WH-WL]
factor<-factor[,.(jdate,ROA=WHML,ROAH=WH,ROAL=WL)]

write.csv(factor,file="Roa_revised.csv")