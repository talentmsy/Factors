...
data.comp.fundq<-data.comp.fundq[,.(gvkey,datadate,fqtr,rdq,year,ajexq,epspxq)]
...
data.comp.fundq[fqtr==lagfqtr,delta:=saqeps-lag(saqeps,4),by=gvkey]
data.comp.fundq[,sddelta:=roll_sd(delta,width = 8,min_obs = 6),by=gvkey]
...
factor[,WL:=(BL+SL)/2]
factor[,WHML:=WH-WL]
factor<-factor[,.(jdate,SUE=WHML,SUEH=WH,SUEL=WL)]

write.csv(factor,file="SUE_revised.csv")
