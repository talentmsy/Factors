...
data.comp.funda<-data.comp.funda[sale>10]
data.comp.funda[,year:=year(datadate)]
data.comp.funda[,ce:=capx/sale]
data.comp.funda<-data.comp.funda[is.na(ce)==FALSE]
data.comp.funda[,aci:=(ce/((lag(ce,1)+lag(ce,2)+lag(ce,3))/3))-1,gvkey]
comp<-data.comp.funda[is.na(aci)==FALSE]
# number of years in Compustat
##why counting years?
comp<-comp[order(gvkey,datadate)]
comp[,count:=ave(gvkey==gvkey, gvkey, FUN=cumsum)]
comp[,count:=count-1]
comp<-comp[,.(gvkey,datadate,year,aci,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,aci,count)]
...
nyse_bm1<-nyse[,quantile(aci,.1),by=jdate]
nyse_bm2<-nyse[,quantile(aci,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(aci<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(aci>bm30&aci<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(aci>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,ACIH=WH,ACIL=WL,ACI=WHML)]
write.csv(fffactor,file="Aci_revised.csv")
