...
data.comp.funda<-data.comp.funda[!is.na(ajex)&!ajex==0]
data.comp.funda[,year:=year(datadate)]
data.comp.funda[,saso:=csho*ajex,by=gvkey]
data.comp.funda[,NSI:=log(saso/(lag(saso,1))),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(NSI)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,NSI,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,NSI,count)]
...
nyse_bm1<-nyse[,quantile(NSI,.1),by=jdate]
nyse_bm2<-nyse[,quantile(NSI,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(NSI<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(NSI>bm30&NSI<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(NSI>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,NSIH=WH,NSIL=WL,NSI=WHML)]
write.csv(factor,file="Nsi_revised.csv")
