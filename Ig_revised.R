...
data.comp.funda<-data.comp.funda[is.na(capx)==FALSE&!capx==0]
data.comp.funda[,ig:=(capx-lag(capx,1))/lag(capx,1),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(ig)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,ig,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,ig,count)]
...
nyse_bm1<-nyse[,quantile(ig,.1),by=jdate]
nyse_bm2<-nyse[,quantile(ig,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(ig<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ig>bm30&ig<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ig>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,IGH=WH,IGL=WL,IG=WHML)]
write.csv(factor,file="Ig_revised.csv")
