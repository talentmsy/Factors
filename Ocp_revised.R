...
data.comp.funda[,opc:=ifelse(year<1988,fopt-(wcap-lag(wcap)),oancf),gvkey]
comp<-data.comp.funda[opc>0]
# number of years in Compustat
comp<-comp[order(gvkey,datadate)]
comp[,count:=ave(gvkey==gvkey, gvkey, FUN=cumsum)]
comp[,count:=count-1]
comp<-comp[,.(gvkey,datadate,year,opc,count)]
...
#ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,td,oibdp,count)]

# link comp and crsp
ccm_jun<-ccm2[crsp_jun,on=.(permno,jdate),nomatch=0]
# define e/p
ccm_jun<-ccm_jun[,nop:=opc/dec_me]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,OPCH=WH,OPCL=WL,OPC=WHML)]
write.csv(factor,file="ocp_revised.csv")
