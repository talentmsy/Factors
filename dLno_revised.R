...
data.comp.funda[,dlno:=((ppent-lag(ppent,1)+intan-lag(intan,1)+ao-lag(ao,1)-lo+lag(lo,1)+dp))/(0.5*(at+lag(at,1))),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(dlno)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,dlno,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,dlno,count)]
...
nyse_bm1<-nyse[,quantile(dlno,.1),by=jdate]
nyse_bm2<-nyse[,quantile(dlno,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(dlno<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dlno>bm30&dlno<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dlno>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dLNOH=WH,dLNOL=WL,dLNO=WHML)]
write.csv(factor,file="dLno_revised.csv")
