...
data.comp.funda[,dwc:=(wc-lag(wc,1))/lag(at,1),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(dwc)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,dwc,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,dwc,count)]
...
nyse_bm1<-nyse[,quantile(dwc,.1),by=jdate]
nyse_bm2<-nyse[,quantile(dwc,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(dwc<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dwc>bm30&dwc<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dwc>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dWCH=WH,dWCL=WL,dWC=WHML)]
write.csv(factor,file="dWc_revised.csv")
