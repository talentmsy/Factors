...
data.comp.funda[,opa:=(revt-cogs-xsga+ifelse(is.na(xrd)==TRUE,0,xrd))/at,by=gvkey]
data.comp.funda<-data.comp.funda[is.na(opa)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,opa,count)]
...
comp1<-comp[,.(gvkey,datadate,opa,count)]
comp1<-comp1[is.na(opa)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,opa,count)]
...
nyse_bm1<-nyse[,quantile(opa,.1),by=jdate]
nyse_bm2<-nyse[,quantile(opa,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(opa<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(opa>bm30&opa<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(opa>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,OPAH=WH,OPAL=WL,OPA=WHML)]
write.csv(factor,file="Opa_revised.csv")
