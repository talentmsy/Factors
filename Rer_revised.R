...
#data.comp.funda<-data.comp.funda[!ppent==0&!ppegt==0]
data.comp.funda[year<1984,rer:=ifelse(is.na(ppent)|ppent==0,NA,(ppenb+ppenls)/ppent)]
data.comp.funda[year>=1984,rer:=ifelse(is.na(ppegt)|ppegt==0,NA,(fatb+fatl)/ppegt)]
data.comp.funda<-data.comp.funda[!is.na(rer)]
data.comp.funda[,sic:=formatC(as.integer(sic),width=4,flag='0')]
data.comp.funda[,nsic:=substr(sic,1,2)]
data.comp.funda[,indaverer:=mean(rer),.(nsic,year)]
data.comp.funda[,numind:=length(unique(gvkey)),by=.(nsic,year)]
data.comp.funda<-data.comp.funda[numind>=5]
data.comp.funda[,arer:=rer-indaverer]
data.comp.funda<-data.comp.funda[is.na(arer)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,arer,count)]
...
comp1<-comp[,.(gvkey,datadate,arer,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,arer,count)]
...
nyse_bm1<-nyse[,quantile(arer,.1),by=jdate]
nyse_bm2<-nyse[,quantile(arer,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(arer<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(arer>bm30&arer<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(arer>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,RERH=WH,RERL=WL,RER=WHML)]
write.csv(factor,file="Rer_revised.csv")
