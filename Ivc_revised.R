...
data.comp.funda<-data.comp.funda[!invt==0&is.na(invt)==FALSE]
data.comp.funda[,ivc:=((invt-lag(invt,1)))/(0.5*(at+lag(at,1))),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(ivc)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,ivc,count)]
...
nyse_bm1<-nyse[,quantile(ivc,.1),by=jdate]
nyse_bm2<-nyse[,quantile(ivc,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(ivc<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ivc>bm30&ivc<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ivc>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,IVCH=WH,IVCL=WL,IVC=WHML)]

write.csv(factor,file="Ivc_revised.csv")
