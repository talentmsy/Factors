...
data.comp.funda[,dFin:=(Fin-lag(Fin))/lag(at),gvkey]

data.comp.funda<-data.comp.funda[is.na(dFin)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,dFin,count)]
...
comp1<-comp[,.(gvkey,datadate,dFin,count)]
comp1<-comp1[is.na(dFin)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,dFin,count)]
...
nyse_bm1<-nyse[,quantile(dFin,.1),by=jdate]
nyse_bm2<-nyse[,quantile(dFin,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(dFin<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dFin>bm30&dFin<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dFin>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dFINH=WH,dFINL=WL,dFIN=WHML)]
write.csv(factor,file="dFin_revised.csv")
