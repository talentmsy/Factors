...
comp<-data.comp.funda[,.(gvkey,datadate,year,be,ib,count)]
...
comp1<-comp[,.(gvkey,datadate,be,ib,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,ib,be,count)]

# link comp and crsp
ccm_jun<-ccm2[crsp_jun,on=.(permno,jdate),nomatch=0]
# define e/p
ccm_jun<-ccm_jun[,ep:=ib*1000/dec_me]
...
write.csv(factor,file="Ep_revised.csv")
