setwd("C:\\Users\\Siyuan\\Desktop\\test")
...
data.comp.funda<-data.comp.funda[is.na(capxv)==FALSE]
data.comp.funda[,sic:=as.integer(sic)]
data.comp.funda[,sic:=formatC(sic,width=4,flag='0')]
data.comp.funda[,sic:=substr(sic,1,2)]
data.comp.funda[,numofind:=length(unique(gvkey)),.(year(datadate),sic)]
data.comp.funda<-data.comp.funda[numofind>=2]
data.comp.funda[,year:=substr(datadate,1,4)]
data.comp.funda[,Einv:=0.5*(lag(capxv,1)+lag(capxv,2)),gvkey]
data.comp.funda<-data.comp.funda[Einv>0]
data.comp.funda[,din:=(capxv-Einv)/Einv,gvkey]
setkeyv(data.comp.funda,c("sic","year"))

data.comp.funda[,ind:=sum(capxv),.(sic,year)]
data.comp.funda[,Einv_ind:=0.5*(lag(ind,1)+lag(ind,2)),sic]
data.comp.funda[,dind:=(ind-Einv_ind)/Einv_ind,sic]
data.comp.funda[,dIi:=din-dind]
data.comp.funda<-data.comp.funda[is.na(dIi)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,sic,capxv,datadate,year,dIi,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,dIi,count)]
...
nyse_bm1<-nyse[,quantile(dIi,.1),by=jdate]
nyse_bm2<-nyse[,quantile(dIi,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(dIi<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dIi>bm30&dIi<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dIi>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dIIH=WH,dIIL=WL,dII=WHML)]
write.csv(factor,file="dIi_revised.csv")
