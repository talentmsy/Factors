...
data.comp.funda<-data.comp.funda[is.na(invt)==FALSE]
data.comp.funda[,ivg:=(invt-lag(invt,1))/lag(invt,1),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(ivg)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,ivg,count)]
...
comp1<-comp[,.(gvkey,datadate,ivg,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,ivg,count)]
...
nyse_bm1<-nyse[,quantile(ivg,.1),by=jdate]
nyse_bm2<-nyse[,quantile(ivg,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(ivg<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ivg>bm30&ivg<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ivg>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,IVGH=WH,IVGL=WL,IVG=WHML)]

write.csv(factor,file="Ivg_revised.csv")
