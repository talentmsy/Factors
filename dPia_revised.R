...
data.comp.funda[,dinppegt:=ppegt-lag(ppegt,1),by=gvkey]
data.comp.funda[,dininvt:=invt-lag(invt,1),by=gvkey]
data.comp.funda[,pia:=(dinppegt+dininvt)/(lag(at,1)),by=gvkey]
comp<-data.comp.funda[is.na(pia)==FALSE]
# number of years in Compustat
comp<-comp[order(gvkey,datadate)]
comp[,count:=ave(gvkey==gvkey, gvkey, FUN=cumsum)]
comp[,count:=count-1]
comp<-comp[,.(gvkey,datadate,year,pia,count)]
...
comp1<-comp[,.(gvkey,datadate,pia,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,pia,count)]
...
nyse_bm1<-nyse[,quantile(pia,.1),by=jdate]
nyse_bm2<-nyse[,quantile(pia,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(pia<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(pia>bm30&pia<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(pia>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dPIAH=WH,dPIAL=WL,dPIA=WHML)]
write.csv(factor,file="dPia.csv")
