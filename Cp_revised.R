...
##########################################################################################################
data.comp.funda[,cp:=ib+dp]
data.comp.funda<-data.comp.funda[cp>0]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,be,cp,count)]
...
comp1<-comp[,.(gvkey,datadate,be,cp,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,cp,be,count)]

# link comp and crsp
ccm_jun<-ccm2[crsp_jun,on=.(permno,jdate),nomatch=0]
# define e/p
ccm_jun<-ccm_jun[,ep:=cp*1000/dec_me]

# select NYSE stocks for bucket breakdown
# exchcd = 1 and positive beme and positive me and shrcd in (10,11) and at least 2 years in comp
nyse<-ccm_jun[exchcd==1&is.na(ep)==FALSE&me>0&count>1&(shrcd==10|shrcd==11)]
# size breakdown
nyse_sz<-nyse[,median(me),by=jdate]
setnames(nyse_sz,"V1","sizemedn")
# beme breakdown

nyse_bm1<-nyse[,quantile(ep,.1),by=jdate]
nyse_bm2<-nyse[,quantile(ep,.9),by=jdate]
...
# join back size and ep breakdown
...
ccm1_jun[is.na(ep)==FALSE&me>0&count>=1,szport:=ifelse(me<=sizemedn,"S","B")]
ccm1_jun[is.na(szport)==TRUE,szport:=""]
# assign book-to-market portfolio
ccm1_jun[is.na(ep)==FALSE&me>0&count>=1,bmport:=ifelse(ep<=bm30,"L","")]
ccm1_jun[is.na(ep)==FALSE&me>0&count>=1,bmport:=ifelse(ep>bm30&ep<=bm70,"M",bmport)]
ccm1_jun[is.na(ep)==FALSE&me>0&count>=1,bmport:=ifelse(ep>bm70,"H",bmport)]
...
factor<-factors[,.(jdate,WH,WL,WHML)]
write.csv(factor,file="Cp_revised.csv")
