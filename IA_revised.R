...
data.comp.funda[,ia:=(at/(lag(at,1)))-1,by=gvkey]
data.comp.funda<-data.comp.funda[is.na(ia)==FALSE]
comp<-data.comp.funda[order(gvkey,datadate)]
comp[,count:=ave(gvkey==gvkey, gvkey, FUN=cumsum)]
comp[,count:=count-1]
comp<-comp[,.(gvkey,datadate,year,ia,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,ia,count)]
...
nyse_bm1<-nyse[,quantile(ia,.1),by=jdate]
nyse_bm2<-nyse[,quantile(ia,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(ia<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ia>bm30&ia<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ia>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,IAH=WH,IAL=WL,IA=WHML)]
write.csv(factor,file="Ia_revised.csv")
