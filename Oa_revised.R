...
data.comp.funda<-data.comp.funda[!is.na(at)&!at==0]
...
data.comp.funda[,OA:=oa/lag(at,1),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(OA)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,OA,count)]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,OA,count)]
...
nyse_bm1<-nyse[,quantile(OA,.1),by=jdate]
nyse_bm2<-nyse[,quantile(OA,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(OA<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(OA>bm30&OA<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(OA>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,OAH=WH,OAL=WL,OA=WHML)]
write.csv(factor,file="Oa_revised.csv")
