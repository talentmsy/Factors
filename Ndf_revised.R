...
data.comp.funda[,df:=dltis-dltr+ifelse(is.na(dlcch-lag(dlcch,1)),0,abs(dlcch-lag(dlcch,1))),by=gvkey]
data.comp.funda[,ndf:=df/(0.5*(lag(at,1)+at)),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(ndf)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,ndf,count)]
...
comp1<-comp[,.(gvkey,datadate,ndf,count)]
comp1<-comp1[is.na(ndf)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,ndf,count)]
...
nyse_bm1<-nyse[,quantile(ndf,.1),by=jdate]
nyse_bm2<-nyse[,quantile(ndf,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(ndf<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ndf>bm30&ndf<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ndf>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,NDFH=WH,NDFL=WL,NDF=WHML)]
write.csv(factor,file="Ndf_revised.csv")
