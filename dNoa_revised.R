...
data.comp.funda[,year:=year(datadate)]
data.comp.funda<-data.comp.funda[is.na(at)==FALSE&at!=0&at>0]
data.comp.funda[,opat:=at-che,by=gvkey]

data.comp.funda[,dlc:=ifelse(is.na(dlc)==TRUE,0,dlc)]
data.comp.funda[,pstk:=ifelse(is.na(pstk)==TRUE,0,pstk)]
data.comp.funda[,dltt:=ifelse(is.na(dltt)==TRUE,0,dltt)]
data.comp.funda[,mib:=ifelse(is.na(mib)==TRUE,0,mib)]
data.comp.funda[,opl:=at-dlc-dltt-mib-pstk-ceq,by=gvkey]

data.comp.funda[,noa:=(opat-opl)/(lag(at,1)),by=gvkey]
data.comp.funda[,dnoa:=((opat-opl)-(lag(opat,1)-lag(opl,1)))/(lag(at,1)),by=gvkey]
data.comp.funda<-data.comp.funda[is.na(dnoa)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,dnoa,count)]
...
comp1<-comp[,.(gvkey,datadate,dnoa,count)]
comp1<-comp1[is.na(dnoa)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,dnoa,count)]
...
nyse_bm1<-nyse[,quantile(dnoa,.1),by=jdate]
nyse_bm2<-nyse[,quantile(dnoa,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(dnoa<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dnoa>bm30&dnoa<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dnoa>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dNOAH=WH,dNOAL=WL,dNOA=WHML)]
write.csv(factor,file="dNoa_revised.csv")
