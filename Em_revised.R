...
##########################################################################################################
data.comp.funda[,td:=dlc+dltt+pstkrv-che]
data.comp.funda[,npd:=td]
...
comp[,count:=count-1]
comp<-comp[,.(gvkey,datadate,year,td,count,oibdp)]
...
# define e/p
ccm_jun<-ccm_jun[,nop:=(td+dec_me)/oibdp]
ccm_jun<-ccm_jun[nop>0|oibdp>0]
...
factor<-factors[,.(jdate=jdate,EMH=WH,EML=WL,EM=WHML)]
write.csv(factor,file="Em_revised.csv")
