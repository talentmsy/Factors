...
data.comp.funda[,ol:=(cogs+xsga)/at,by=gvkey]
data.comp.funda<-data.comp.funda[is.na(ol)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,ol,count)]
...
comp1<-comp[,.(gvkey,datadate,ol,count)]
comp1<-comp1[is.na(ol)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,ol,count)]
...
nyse_bm1<-nyse[,quantile(ol,.1),by=jdate]
nyse_bm2<-nyse[,quantile(ol,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(ol<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ol>bm30&ol<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(ol>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,OLH=WH,OLL=WL,OL=WHML)]
write.csv(factor,file="=Ol_revised.csv")
