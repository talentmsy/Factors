...
data.comp.funda<-data.comp.funda[is.na(at)==FALSE&!at==0]
...
data.comp.funda[,dFnl:=(Fnl-lag(Fnl))/lag(at),gvkey]

data.comp.funda<-data.comp.funda[is.na(dFnl)==FALSE]
...
comp<-data.comp.funda[,.(gvkey,datadate,year,dFnl,count)]
...
comp1<-comp[,.(gvkey,datadate,dFnl,count)]
comp1<-comp1[is.na(dFnl)==FALSE]
...
ccm2<-ccm2[,.(gvkey,permno,datadate,yearend,jdate,dFnl,count)]
...
nyse_bm1<-nyse[,quantile(dFnl,.1),by=jdate]
nyse_bm2<-nyse[,quantile(dFnl,.9),by=jdate]
...
ccm1_jun[me>0&count>=1,bmport:=ifelse(dFnl<=bm30,"L","")]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dFnl>bm30&dFnl<=bm70,"M",bmport)]
ccm1_jun[me>0&count>=1,bmport:=ifelse(dFnl>bm70,"H",bmport)]
...
factors[,WL:=(BL+SL)/2]
factors[,WHML:=WH-WL]
factor<-factors[,.(jdate=jdate,dFNLH=WH,dFNLL=WL,dFNL=WHML)]
write.csv(factor,file="dFnl_revised.csv")
